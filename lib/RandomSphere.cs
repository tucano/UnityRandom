...
namespace URandom
{
	public static class RandomSphere
	{
		...
		private static Vector3 PickCubePoints( ref NPack.MersenneTwister _rand )
		{
			...
			float x = SpecialFunctions.ScaleFloatToRange( _rand.NextSingle(), -1, 1, 0, 1 );
			float y = SpecialFunctions.ScaleFloatToRange( _rand.NextSingle(), -1, 1, 0, 1 );
			float z = SpecialFunctions.ScaleFloatToRange( _rand.NextSingle(), -1, 1, 0, 1 );			
